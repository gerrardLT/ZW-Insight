-- V2026_25: 项目结项/关闭审批流程 - biz_project 补充 workflow_instance_id 字段
--
-- 背景：
...
--   需要在 biz_project 上记录关联的流程实例ID(workflow_instance_id)，与其他走审批的业务表(biz_project_settlement 等)保持一致。
-- 方案：用 information_schema 判存在性，不存在才 ADD COLUMN（幂等，可重复执行）；同时更新 status 列注释以纳入 CLOSING。
