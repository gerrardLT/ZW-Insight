...
/**
 * {@link TenderProjectCascadeCleanupListener} 单元测试（R7-02）。
 * <p>钉住两条核心契约：</p>
 * <ol>
...
@DisplayName("项目删除级联清理监听器（tender 模块）")
class TenderProjectCascadeCleanupListenerTest {
    ...
    @Mock private BizTenderFeeMapper tenderFeeMapper;
    @Mock private BizOpenBidRecordMapper openBidRecordMapper;

    @InjectMocks
    private TenderProjectCascadeCleanupListener listener;
    ...
    void onProjectDeleted_cleansAllOwnedTablesScopedToProject() {
        ...
        verify(tenderRegisterMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(depositApplyMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(tenderFeeMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(openBidRecordMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
    }

    @Test
    @DisplayName("异常路径：清理失败必须向上抛出以触发整体回滚（不吞异常）")
    void onProjectDeleted_mapperFailure_propagates() {
        when(tenderRegisterMapper.delete(any())).thenThrow(new RuntimeException("DB write timeout"));
