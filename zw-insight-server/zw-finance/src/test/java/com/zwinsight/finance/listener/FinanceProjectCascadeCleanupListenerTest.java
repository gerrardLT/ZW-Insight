...
/**
 * {@link FinanceProjectCascadeCleanupListener} 单元测试（R7-02）。
 * <p>钉住两条核心契约：</p>
 * <ol>
...
@DisplayName("项目删除级联清理监听器（finance 模块）")
class FinanceProjectCascadeCleanupListenerTest {
    ...
    @Mock private BizProjectSettlementMapper projectSettlementMapper;
    @Mock private BizRetentionMoneyMapper retentionMoneyMapper;
    @Mock private BizBankAccountMapper bankAccountMapper;
    @Mock private BizOtherPaymentMapper otherPaymentMapper;
    @Mock private BizProjectReimbursementMapper projectReimbursementMapper;
    @Mock private BizReserveFundApplyMapper reserveFundApplyMapper;
    @Mock private BizFinanceLockMapper financeLockMapper;

    @InjectMocks
    private FinanceProjectCascadeCleanupListener listener;
    ...
    void onProjectDeleted_cleansAllOwnedTablesScopedToProject() {
        ...
        verify(paymentApplyMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(paymentReceivedMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(invoiceApplyMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(invoiceReceivedMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(projectSettlementMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(retentionMoneyMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(bankAccountMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(otherPaymentMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(projectReimbursementMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(reserveFundApplyMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(financeLockMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
    }

    @Test
    @DisplayName("异常路径：清理失败必须向上抛出以触发整体回滚（不吞异常）")
    void onProjectDeleted_mapperFailure_propagates() {
        when(paymentApplyMapper.delete(any())).thenThrow(new RuntimeException("DB write timeout"));
