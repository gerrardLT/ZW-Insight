...
/**
 * {@link MachineProjectCascadeCleanupListener} 单元测试（R7-02）。
 * <p>钉住两条核心契约：</p>
 * <ol>
...
@DisplayName("项目删除级联清理监听器（machine 模块）")
class MachineProjectCascadeCleanupListenerTest {

    private static final Long PROJECT_ID = 90001L;

    @Mock private BizMachineContractMapper machineContractMapper;
    @Mock private BizMachineEntryMapper machineEntryMapper;
    @Mock private BizMachineWorkLogMapper machineWorkLogMapper;
    @Mock private BizMachineWorkSettlementMapper machineWorkSettlementMapper;
    ...
    @Mock private BizMachineOilRecordMapper machineOilRecordMapper;
    @Mock private BizMachineRepairMapper machineRepairMapper;

    @InjectMocks
    private MachineProjectCascadeCleanupListener listener;
    ...
    void onProjectDeleted_cleansAllOwnedTablesScopedToProject() {
        ...
        verify(machineContractMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(machineEntryMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(machineWorkLogMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(machineWorkSettlementMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(machineSettlementMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(machineUsageRecordMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(machineOilRecordMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(machineRepairMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
    }

    @Test
    @DisplayName("异常路径：清理失败必须向上抛出以触发整体回滚（不吞异常）")
    void onProjectDeleted_mapperFailure_propagates() {
        when(machineContractMapper.delete(any())).thenThrow(new RuntimeException("DB write timeout"));
