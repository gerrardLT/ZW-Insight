...
/**
 * {@link BudgetProjectCascadeCleanupListener} 单元测试（R7-02）。
 * <p>钉住两条核心契约：</p>
 * <ol>
...
@DisplayName("项目删除级联清理监听器（budget 模块）")
class BudgetProjectCascadeCleanupListenerTest {

    private static final Long PROJECT_ID = 90001L;

    @Mock private BizBudgetMapper budgetMapper;
    @Mock private BizBudgetChangeMapper budgetChangeMapper;
    @Mock private BizBudgetConfigMapper budgetConfigMapper;
    @Mock private BizCostAccountMapper costAccountMapper;
    @Mock private BizCostAccountLinkMapper costAccountLinkMapper;
    @Mock private BizCostAccountTxnMapper costAccountTxnMapper;
    @Mock private SysBudgetControlConfigMapper budgetControlConfigMapper;

    @InjectMocks
    private BudgetProjectCascadeCleanupListener listener;
    ...
    void onProjectDeleted_cleansAllOwnedTablesScopedToProject() {
        ...
        verify(budgetMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(budgetChangeMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(budgetConfigMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(costAccountMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(costAccountLinkMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(costAccountTxnMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(budgetControlConfigMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
    }

    @Test
    @DisplayName("异常路径：清理失败必须向上抛出以触发整体回滚（不吞异常）")
    void onProjectDeleted_mapperFailure_propagates() {
        when(budgetMapper.delete(any())).thenThrow(new RuntimeException("DB write timeout"));
