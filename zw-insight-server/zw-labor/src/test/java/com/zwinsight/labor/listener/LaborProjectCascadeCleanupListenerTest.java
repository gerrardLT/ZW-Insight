...
/**
 * {@link LaborProjectCascadeCleanupListener} 单元测试（R7-02）。
 * <p>钉住两条核心契约：</p>
 * <ol>
...
@DisplayName("项目删除级联清理监听器（labor 模块）")
class LaborProjectCascadeCleanupListenerTest {

    private static final Long PROJECT_ID = 90001L;

    @Mock private BizLaborContractMapper laborContractMapper;
    @Mock private BizLaborSettlementMapper laborSettlementMapper;
    @Mock private BizLaborPayrollMapper laborPayrollMapper;
    @Mock private BizLaborOutputReportMapper laborOutputReportMapper;
    @Mock private BizLaborRosterMapper laborRosterMapper;
    @Mock private BizLaborRewardPunishMapper laborRewardPunishMapper;
    @Mock private BizTeamMapper teamMapper;
    @Mock private BizWorkOrderMapper workOrderMapper;

    @InjectMocks
    private LaborProjectCascadeCleanupListener listener;
    ...
    void onProjectDeleted_cleansAllOwnedTablesScopedToProject() {
        ...
        verify(laborContractMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(laborSettlementMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(laborPayrollMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(laborOutputReportMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(laborRosterMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(laborRewardPunishMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(teamMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(workOrderMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
    }

    @Test
    @DisplayName("异常路径：清理失败必须向上抛出以触发整体回滚（不吞异常）")
    void onProjectDeleted_mapperFailure_propagates() {
        when(laborContractMapper.delete(any())).thenThrow(new RuntimeException("DB write timeout"));
