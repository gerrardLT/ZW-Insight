...
/**
 * {@link SubcontractProjectCascadeCleanupListener} 单元测试（R7-02）。
 * <p>钉住两条核心契约：</p>
 * <ol>
...
@DisplayName("项目删除级联清理监听器（subcontract 模块）")
class SubcontractProjectCascadeCleanupListenerTest {
    ...
    @Mock private BizSubcontractOutputReportMapper subcontractOutputReportMapper;
    @Mock private BizSubcontractRewardPunishMapper subcontractRewardPunishMapper;

    @InjectMocks
    private SubcontractProjectCascadeCleanupListener listener;
    ...
    void onProjectDeleted_cleansAllOwnedTablesScopedToProject() {
        ...
        verify(subcontractMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(subcontractSettlementMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(subcontractOutputReportMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
        verify(subcontractRewardPunishMapper).delete(argThat(w -> isProjectScoped(w, PROJECT_ID)));
    }

    @Test
    @DisplayName("异常路径：清理失败必须向上抛出以触发整体回滚（不吞异常）")
    void onProjectDeleted_mapperFailure_propagates() {
        when(subcontractMapper.delete(any())).thenThrow(new RuntimeException("DB write timeout"));
