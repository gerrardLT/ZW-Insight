...
### 6.5 线上实证结果（提交 `026c05c`，CI run `35870252510` 部署成功）

**迁移**：V2026_63 OK @22:13:32，`overdue_unpaid` 列 `decimal(18,2)` 已建。

**金额对账（核心验证）**：

| 项 | 修复前 | 修复后（实测） |
|---|---|---|
| 库内逾期事实源 | 16 条 / 58,500,000 | 同（不变） |
| 当月 `expected_payments` | 0 | **58,500,000** |
| 当月 `overdue_unpaid` | —（无此列） | **58,500,000** |
| 当月 `net_gap` | −2,600,000（盈余） | **+55,900,000** |
| 当月 `risk_level` | LOW | **HIGH**（收款覆盖率 4.4% < 50%） |
| 后续 5 个月 `overdue_unpaid` | — | **均为 0**（不摊开 ✓） |
| SQL 对账断言 | — | **MATCH 逾期已如实计入** |

**端到端链路打通**：风险台账新增 `FUND_GAP` **RED** 风险（impact 55,900,000，`reason_detail` 完整记录 month/netGap/expectedPayments/expectedReceipts/forecastRiskLevel）。**修复前该风险根本不会产生**（net_gap 为负→规则不触发），老板完全看不到 5590 万资金压力。

**回归验证**：

| 项 | 结果 |
|---|---|
| L3 `test-api-fund-loop.sh` | **32 通过 / 0 失败**（原 21 项 + 新增 11 项逾期口径断言全绿） |
| L3 `test-api-risk.sh` | **36 通过 / 0 失败 / 1 跳过**（跳过项仍为利润归因，待 03:30 快照任务） |
| 数据审计 R7 回归 | **PASS=65 / FAIL=0 / WARN=0 / INFO=40**（与基线一致，全部勾稽 0 MISMATCH） |
| CI 流水线 | Backend Build / 三端前端单测 / Deploy to Server 均 success |

**本节结论**：两个缺陷已上线修复并经真金白银对账验证；缺陷 C 属会直接误导经营决策的严重口径错误（将 5590 万待付缺口显示为 260 万盈余），幸而在上线当天被发现。
