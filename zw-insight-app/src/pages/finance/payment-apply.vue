<template>
  <ZwiFormPage submitText="提交申请" :loading="submitting" @submit="handleSubmit">
    ...
    <view class="form-card">
      <ZwiField label="付款金额(元)" v-model="form.amount" inputType="digit" placeholder="请输入付款金额" />
      <ZwiField label="收款方" v-model="form.payee" placeholder="请输入收款方名称" />
      <ZwiField label="收款账号" v-model="form.payeeAccount" placeholder="请输入收款账号" />
      <ZwiField label="开户行" v-model="form.payeeBank" placeholder="请输入开户行" />
      <ZwiField label="付款事由" v-model="form.reason" placeholder="如：材料采购款" />
      ...
      <ZwiField label="期望付款日" v-model="form.expectedDate" placeholder="YYYY-MM-DD" />
