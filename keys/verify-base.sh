...
# ---------------------------------------------------------------------------
# clear_login_locks：清除登录失败计数/锁定状态键（仅测试服务器环境重置）
#   背景（2026-08-13 事故）：L3/L4 串行脚本共享同一来源 IP（127.0.0.1），
#   任一脚本的登录失败（含负向测试的故意失败）计入 IP 失败计数，
#   5 分钟窗口内累达 5 次即锁 15 分钟，连锁拖垮后续全部脚本登录。
#   锁定机制本身有效（该事故即实证），此处仅为测试环境重置状态键，
#   不削弱生产安全：攻击流量在真实环境仍会被锁定。
# ---------------------------------------------------------------------------
clear_login_locks() {
