    ...
    <el-card shadow="never">
      <el-form :model="queryParams" inline>
        ...
        <el-form-item label="供应商">
          <el-input v-model="queryParams.supplierName" placeholder="供应商名称" clearable style="width: 180px" />
        </el-form-item>
      ...
      <div class="table-toolbar">
        <el-button type="primary" @click="handleAdd">新增采购合同</el-button>
      </div>
...
async function loadData() {
  loading.value = true
  try {
    const res: any = await getPurchaseContractPage(queryParams.value)
...
async function handleFormSubmit() {
  ...
  try {
    isEdit.value ? await updatePurchaseContract(formData.value) : await createPurchaseContract(formData.value)
...
<style scoped>
.purchase-contract-container { padding: var(--zw-space-md); }
