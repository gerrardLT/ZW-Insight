  ...
  <div class="vehicle-container">
    <el-card shadow="never">
      <el-form :model="queryParams" inline>
        <el-form-item label="车牌号">
          <el-input v-model="queryParams.plateNo" placeholder="车牌号" clearable style="width: 140px" />
        </el-form-item>
      ...
      <el-table :data="tableData" v-loading="loading" border>
        ...
        <el-table-column label="状态" width="80" align="center">
          <template #default="{ row }">
            <el-tag :type="row.status === 'NORMAL' ? 'success' : 'danger'" size="small">{{ row.status === 'NORMAL' ? '正常' : '维修' }}</el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑车辆' : '新增车辆'" width="550px" destroy-on-close>
      <el-form ref="formRef" :model="formData" :rules="formRules" label-width="90px">
        <el-form-item label="车牌号" prop="plateNo"><el-input v-model="formData.plateNo" /></el-form-item>
...
const queryParams = ref({ pageNum: 1, pageSize: 10, plateNo: '', vehicleType: '' })
const formData = ref({ id: undefined as number | undefined, plateNo: '', vehicleType: '', brand: '', driver: '', department: '', insuranceExpiry: '', inspectionExpiry: '' })
const formRules = { plateNo: [{ required: true, message: '请输入车牌号', trigger: 'blur' }] }
...
function handleReset() { queryParams.value = { pageNum: 1, pageSize: 10, plateNo: '', vehicleType: '' }; loadData() }
function handleAdd() { isEdit.value = false; formData.value = { id: undefined, plateNo: '', vehicleType: '', brand: '', driver: '', department: '', insuranceExpiry: '', inspectionExpiry: '' }; dialogVisible.value = true }
