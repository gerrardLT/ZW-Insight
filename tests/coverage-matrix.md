...
| ~~材料退货出库（含退货退款分支）~~ ✅ 批 5 完成（阶段 9K，CI run 31454634286 绿，L4 88/88） | 2.5 | 中 |
| ~~备用金申请→报销冲抵→归还~~ ✅ 批 5 完成（阶段 9I 申请→归还回写 CI 绿；冲抵由 L1 ReserveFundReturnServiceTest 覆盖） | 2.9 | 中（金额） |
| ~~保证金退回登记（投标侧）~~ ✅ 批 5 完成（阶段 9J，CI run 31454634286 绿） | 1.4 | 中（金额） |
