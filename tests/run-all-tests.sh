...
# 入口
# ===========================================================================
# Source-guard：仅当脚本被「直接执行」时才自动运行 main；被 `source` 引入时不自动执行，
# 以便属性测试（test-run-all-properties.sh）注入 mock 的 run_l1..run_l5 后显式调用 main，
# 从而在不触发真实 mvn/playwright 的前提下验证编排逻辑（--layers / --fail-fast / 报告不变量）。
# 直接执行 `bash run-all-tests.sh` 的行为与本改动前完全一致（零行为变更）。
