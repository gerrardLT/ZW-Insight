...
flowchart TB
    ...
    subgraph 决策层["归集分析与决策层"]
        ...
        E2["🆕 滚动预测重做<br/>付款侧=已批未付(现金口径)<br/>收款侧=应收台账到期<br/>FundForecastTask 每日01:15刷新"]
        E3["老板资金看板 ✅ 垫资/现金流/回款率/缺口"]
        E4["🆕 预计利润快照 biz_profit_snapshot<br/>ProfitSnapshotTask 每日02:30<br/>+ 环比归因（按成本类别分解）"]
        E5["🆕 统一风险台账 biz_risk_register<br/>7条规则 RiskScanTask 每日02:00<br/>六要素+处理状态流转"]
        E6["🆕 经营驾驶舱 /cockpit<br/>经营总览·资金中心·风险中心"]
        E7["月度经营分析表 ❌ 未实现<br/>单据穿透链 ⚠️ 首批已通"]
    end
...
4. 月度计划明细合计 = 主表 `expense_plan`（容差 0.01），不一致抛异常，不静默落库。
...
| 侧 | 改造前 | 改造后 |
|---|---|---|
| 付款 | `status=APPROVED` 的付款申请按付款日落月（注释称"已审批未付"，但该状态当时不存在） | `status=APPROVED` **且 `pay_status≠PAID`**，真实"已批未付" |
...
```text
01:15  FundForecastTask      滚动预测刷新（付款侧依赖 pay_status，收款侧依赖应收台账）
02:00  RiskScanTask          风险扫描（FUND_GAP 规则消费 01:15 的预测快照，必须在其后）
02:30  ProfitSnapshotTask    预计利润快照（PROFIT_LOSS 规则实时计算，不依赖快照）
08:00  RetentionWarningTask  质保金催办（既有）
...
6. **月度经营分析表（资金流转文档 §9）未实现**：数据源已齐（成本账户 + 付款 + 结算 + 计划明细），属计划 P2 项。
7. **单据穿透已通首批链路**：风险详情 → 项目成本主线/项目看板/应收台账/资金计划/质保金/工资专户；未登记类型显式提示“暂无穿透映射”，不伪造跳转。移动端仅对 `PROJECT`/`COST_ACCOUNT` 提供穿透（其余业务页无移动端对应页面）。
